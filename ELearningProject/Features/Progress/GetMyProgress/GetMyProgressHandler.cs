...
{
    public class GetMyProgressHandler : IRequestHandler<GetMyProgressQuery, EndpointResponse<ProgressDto>>
    {
        private readonly IUnitOfWork _unitOfWork;

        public GetMyProgressHandler(IUnitOfWork unitOfWork)
        {
            _unitOfWork = unitOfWork;
        }

        public async Task<EndpointResponse<ProgressDto>> Handle(GetMyProgressQuery request, CancellationToken cancellationToken)
        {
            ...
            if (!isAdmin)
            {
                var isInBatch = await batchStudentRepo
                    .FindByCondition(bs => bs.BatchId == track.BatchId && bs.StudentId == request.StudentId)
                    .AnyAsync(cancellationToken);

                if (!isInBatch)
                    return EndpointResponse<ProgressDto>.ErrorResponse("You are not enrolled in the batch for this track.", 403);
            }

            // 3. CompletionPercentage - calculate from finalized submissions per assignment
            ...
            // 4. AverageScore - live from all finalized submissions in batch
            ...
            // 5. Rank - compare against all students in the same batch
